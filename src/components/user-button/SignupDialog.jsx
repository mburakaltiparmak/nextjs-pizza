    ...
    return (
        <AlertDialog open={signupOpen} onOpenChange={setSignupOpen}>
            <AlertDialogContent className="w-full max-w-sm bg-transparent border-none shadow-none p-0 z-50">
                <VisuallyHidden>
                    <AlertDialogTitle>Hesap Oluştur</AlertDialogTitle>
                    <AlertDialogDescription>Kayıt Formu</AlertDialogDescription>
                </VisuallyHidden>

                <div className="w-full bg-white rounded-2xl shadow-2xl border-2 border-red overflow-hidden relative">
                    <button
                        onClick={() => setSignupOpen(false)}
                        className="absolute right-2 top-2 p-1.5 bg-white/10 hover:bg-white/20 rounded-full transition-colors group z-20"
                        aria-label="Kapat"
                    >
                        <X size={20} className="text-white drop-shadow-md group-hover:text-yellow transition-colors" />
                    </button>

                    {/* Header Section */}
                    <div className="bg-gradient-to-r from-red to-darkred p-5 text-center relative">
                        <div className="w-14 h-14 bg-white rounded-full mx-auto mb-2 flex items-center justify-center shadow-lg">
                            <FontAwesomeIcon icon={faUserPlus} className="text-xl text-red" />
                        </div>
