...
const CategoryPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCategory) {
        // Kategori güncelleme
        response = await instance.put(`/category/${editingCategory.id}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
        addNotification("Kategori başarıyla güncellendi");
      } else {
        // Yeni kategori ekleme
        response = await instance.post('/category', formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
        addNotification("Kategori başarıyla eklendi");
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addNotification(error.response?.data?.message || "Kategori silinirken bir hata oluştu", 'error');
    }
  };
  ...
  return (
    ...
    >
              ...
              <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                <div className="flex space-x-2">
                  ...
                  <button 
                    ...
                    className="p-2 bg-white rounded-full hover:bg-gray-100 transition-colors"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-red">
      ...
      <Modal
        ...
        footer={
          <FormButtons
            ...
            submitText={editingCategory ? 'Güncelle' : 'Kaydet'}
          />
        }
      >
      ...
      <ConfirmationModal
        ...
        warning={categoryToDelete?.products?.length > 0 ? 
          `Bu kategori ${categoryToDelete.products.length} ürün içeriyor. Kategoriyi silmek bu ürünleri de etkileyebilir.` : null
        }
      />
    </AdminLayout>
  );
