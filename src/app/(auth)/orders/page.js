...
export default function OrdersPage() {
  ...
  const reduxOrders = useSelector((state) => {
    ...
    if (!state || !state.order) {
      console.error("Redux state veya order state boş", state);
      return [];
    }
  ...
  // Redux store'daki sorun için debug
  useEffect(() => {
    console.log("Redux orders:", reduxOrders);
    console.log("Local orders:", localOrders);
    console.log("Orders to display:", orders);
    console.log("Order fetch state:", orderFetchState);
  }, [reduxOrders, localOrders, orders, orderFetchState]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (isAuthenticated && !authLoading) {
        try {
          // Bu dispatch fonksiyonu action'ın sonucunu döndürüyor olmalı
          const result = await dispatch(fetchUserOrders());
  ...
  if (!isAuthenticated) {
    return null; // Auth kontrolü zaten useEffect'te yapılıyor, bu sadece koruma amaçlı
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 font-Barlow">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex justify-between mt-1">
                          ...
                          <span className={`px-2 py-0.5 rounded-full text-white text-xs ${
                            order.payment?.paymentStatus === "SUCCESS" ? "bg-green-600" : 
      ...
      {/* Sepet butonunu sadece ADMIN veya PERSONAL olmayan kullanıcılara göster */}
      {showCartButton && <FloatingCartButton />}
    </div>
  );
}