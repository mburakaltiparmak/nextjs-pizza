...
export default function SuccessPage() {
    ...
    const fetchAddressIfNeeded = async () => {
      ...
      if (userData?.addressId && !selectedAddress && !addressLoading) {
        try {
          ...
          console.log("Adres ID ile adres detayları getiriliyor:", userData.addressId);
          
          // API'den adres bilgisini getir
          const response = await instance.get(`user/addresses/${userData.addressId}`);
  ...
  const getOrderStatusText = (status) => {
    switch(status) {
      ...
      case "SHIPPED": return "Yola Çıktı";
      case "DELIVERED": return "Teslim Edildi";
      case "CANCELLED": return "İptal Edildi";
      default: return status || "İşleme Alındı";
    }
  };
  ...
  const renderOrderItems = () => {
    if (latestOrder.items && Array.isArray(latestOrder.items)) {
      ...
      return latestOrder.map((item, index) => (
        <div key={index} className="">
          <div className="">
            <h4 className="font-semibold text-sm">{item.product?.name || 'Ürün'}</h4>
            <span className="text-sm">{item.count} x {item.product?.price} TL</span>
          </div>
    ...
    return <p className="text-center text-yellow">Sipariş öğeleri bulunamadı.</p>;
  };
  ...
  const calculateTotal = () => {
    if (latestOrder.totalAmount) {
      ...
      return latestOrder.items.reduce((total, item) => 
        total + (item.quantity * item.price), 0
      );
    } else if (Array.isArray(latestOrder)) {
      // LatestOrder bir array ise hesapla (cart verisi)
      return latestOrder.reduce((total, item) => 
  ...
  const getDeliveryAddress = () => {
    ...
    if (latestOrder && typeof latestOrder.address === 'string' && latestOrder.address) {
    ...
    if (userData) {
      ...
      if (userData.addressId) {
        console.log("5.3. Sadece userData.addressId bulundu:", userData.addressId);
